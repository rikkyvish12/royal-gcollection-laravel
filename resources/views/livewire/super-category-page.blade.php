<div class="min-h-screen bg-stone-50">
    ...
    <div class="bg-white border-b border-stone-200 py-12">
        <div class="max-w-7xl mx-auto px-6">
            <h1 class="font-serif text-4xl font-bold text-royal-dark uppercase tracking-wider">{{ $superCategory->name }}</h1>
            <p class="text-stone-600 mt-2 text-sm">{{ $superCategory->description }}</p>
        </div>
    </div>
            ...
            <aside class="lg:w-1/4 w-full">
                <div class="bg-white rounded-sm border border-stone-200 sticky top-20">
                    ...
                    <div class="p-6 border-b border-stone-200 flex justify-between items-center">
                        ...
                        @if(!empty($selectedCategories) || !empty($selectedBrands) || $minPrice > 0 || $maxPrice < 1000000)
                        ...
                        <div class="flex flex-wrap gap-2 items-center">
                            ...
                            @foreach($selectedCategories as $categoryId)
                                ...
                                <span class="inline-flex items-center gap-2 px-3 py-1 bg-royal-gold/10 text-royal-dark text-xs rounded-full border border-royal-gold/30">
                                    {{ $cat->name }}
                                    <button wire:click="toggleCategory({{ $categoryId }})" class="hover:text-red-600">
                    ...
                    @forelse($products as $product)
                        <a href="{{ route('product.detail', $product->id) }}" class="group bg-white rounded-sm border border-stone-200 overflow-hidden hover:shadow-xl transition-all duration-300">
                            ...
                            <div class="aspect-square bg-stone-100 relative overflow-hidden">
                                ...
                                <!-- Category Badge -->
                                <div class="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-sm">
                                    <span class="text-[10px] font-bold uppercase tracking-widest text-royal-dark">{{ $product->category->name }}</span>
                                </div>

                                <!-- Brand Badge -->
                                <div class="absolute top-3 left-3 bg-royal-gold/90 backdrop-blur-sm px-3 py-1 rounded-sm">
                                    <span class="text-[10px] font-bold uppercase tracking-widest text-white">{{ $product->brand }}</span>
                                </div>
                            </div>
